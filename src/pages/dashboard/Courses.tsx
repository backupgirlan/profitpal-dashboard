    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-primary text-glow flex items-center gap-2">
          <GraduationCap className="w-6 h-6" /> Curso
        </h1>
        <p className="text-muted-foreground">Aprenda com conteúdos organizados por categoria</p>
      </div>
      ...
      {activeVideo && (
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          <div className="p-4">
            <h3 className="font-display text-sm font-bold text-foreground">{activeVideo.title}</h3>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
          ...
          return (
            <div key={cat.id} className="bg-card border border-border rounded-lg overflow-hidden">
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">{catVideos.length} vídeo{catVideos.length !== 1 ? 's' : ''}</span>
              </button>
              {isOpen && (
                <div className="border-t border-border">
                  {catVideos.length === 0 ? (
                    <p className="p-4 text-center text-muted-foreground text-sm">Nenhum vídeo nesta categoria</p>
                  ) : (
                    catVideos.map((v) => (
                      <button
                        key={v.id}
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-secondary/50 transition-colors border-b border-border/50 last:border-b-0 ${
                          activeVideo?.id === v.id ? 'bg-primary/10 text-primary' : 'text-foreground'
                        }`}
                      >
        ...
        {categories.length === 0 && (
          <div className="text-center text-muted-foreground py-8">Nenhum curso disponível no momento</div>
        )}
      </div>
    </div>
  );
