...
export default function AdminMessagePopup() {
  ...
  useEffect(() => {
    ...
    supabase.from('admin_messages').select('*')
      ...
      .limit(1)
      .then(({ data }) => {
        if (data && data.length > 0) setMessage(data[0]);
      });
  }, [user]);

  const handleClose = async () => {
    if (message) {
      await supabase.from('admin_messages').update({ is_read: true }).eq('id', message.id);
      setMessage(null);
    }
  };
  ...
  return (
    <Dialog open={!!message} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent className="bg-card border-primary/30 box-glow">
        ...
        <Button onClick={handleClose} className="gradient-gold text-primary-foreground font-display">
          Entendido
        </Button>
      </DialogContent>
    </Dialog>
