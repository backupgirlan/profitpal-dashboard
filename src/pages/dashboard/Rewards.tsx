...
const Rewards = () => {
  ...
  useEffect(() => {
    ...
    supabase.from('daily_rewards').select('*').eq('user_id', user.id).eq('reward_date', today)
      .then(({ data }) => {
        if (data && data.length > 0) setTodayClaimed(true);
      });
    // Get recent rewards
  ...
  const claimReward = async () => {
    ...
    if (error) {
      ...
      toast({ title: '🎁 Brinde coletado!', description: `Sequência: ${newStreak} dias!` });
    }
  };
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-primary text-glow flex items-center gap-2">
          <Gift className="w-6 h-6" /> Brindes Diários
        </h1>
        <p className="text-muted-foreground">Ganhe recompensas por logar e registrar resultados todos os dias</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg p-6 text-center box-glow">
        ...
        <Button
          ...
          className="mt-6 gradient-gold text-primary-foreground font-display gap-2"
          size="lg"
        >
          {todayClaimed ? (
            <><CheckCircle className="w-4 h-4" /> Já coletado hoje!</>
          ) : (
            <><Gift className="w-4 h-4" /> Coletar Brinde</>
          )}
        </Button>
      </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {days.map((day) => {
            ...
            return (
              ...
              >
                <p className="text-muted-foreground">{new Date(day + 'T12:00').toLocaleDateString('pt-BR', { weekday: 'short' })}</p>
                <p className={`mt-1 ${claimed ? 'text-primary' : 'text-muted-foreground'}`}>
                  {claimed ? '🎁' : '○'}
                </p>
              </div>
            );
